...
lastmod: 2026-07-08T22:08:47+08:00
...
### 文章头生成技巧：VS Code 代码片段 / Hugo 命令

#### VS Code 代码片段自动生成文章头

直接利用 VS Code 的“用户代码片段”功能，需要自己先手动建好一个空的 .md 文件，然后 `Ctrl` + `Shift` + `P` 敲入 `header` 按 Tab 键生成文件头。
...
```json
{
    "Hugo Front Matter": {
        ...
        "body": [
            "---",
            "title: \"$1\"",
            "date: $CURRENT_YEAR-$CURRENT_MONTH-$CURRENT_DATET$CURRENT_HOUR:$CURRENT_MINUTE:$CURRENT_SECOND+08:00",
            "lastmod: $CURRENT_YEAR-$CURRENT_MONTH-$CURRENT_DATET$CURRENT_HOUR:$CURRENT_MINUTE:$CURRENT_SECOND+08:00",
            "slug: \"$2\"",
            "image: \"$3\"",
            "categories:",
            "  - $4",
...
#### 配置默认文章骨架模板使用 hugo 命令生成文章头

嫌麻烦的话也可以直接使用 hugo new post/"xxxxx.md"直接生成文章，要加上文章发布时间、更新时间等信息，修改默认的模板配置即可：archetypes/default.md（如果没有就去主题目录 themes/hugo-theme-stack/archetypes/ 下复制）
...
这样每次进入博客目录和虚拟环境下后，创建新的文章只需要敲击 hugo 命令回车即可，不需要在 vscode 里呼出命令面板再选择 json 脚本生成文章头。
